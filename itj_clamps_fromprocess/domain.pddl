(define (domain itj_clamp_only)
    ...
    (:action assemble_beam_with_screwdriver_ignored
        :parameters (?beam ?gripper ?grippertype)
        :precondition (and
            ;; Beam is at storage
            (BeamAtStorage ?beam)
            ...
            ;; The beam requires a screwdriver
            (exists (?earlierbeam) (JointNeedsScrewdriver ?earlierbeam ?beam))
            )
        :effect (and
            (not (BeamAtStorage ?beam)) ;; Beam no longer at storage
            (BeamAtAssembled ?beam) ;; Beam now at assembled
    ...
    (:action assemble_scaffolding
        :parameters (?beam)
        :precondition (and
            ;; Beam is at storage
            (BeamAtStorage ?beam)

            ;; Has at least one BeamScaffolding 
            (exists (?earlierbeam)(and
                (BeamScaffolding ?earlierbeam ?beam)
