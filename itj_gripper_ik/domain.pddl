(define (domain itj_gripper_ik)
  ...
  (:predicates
    ...
    ; Motion Checks using streams
    (AssembleBeamIkReachable ?beam ?gripper ?ikresult) ;; Sampled by stream assemble_beam_ik_check
  )
  ...
  (:action assemble_beam
    :parameters (?beam ?gripper ?ikresult)
    :precondition (and
      (GripperAtRobot ?gripper) ;; Gripper is already on robot
      (BeamAtRobot ?beam) ;; Beam is already on robot
      (AssembleBeamIkReachable ?beam ?gripper ?ikresult) ;; AssembleBeamIkReachable is possible 
    )
    :effect (and
